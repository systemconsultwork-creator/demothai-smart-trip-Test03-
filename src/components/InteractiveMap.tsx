...
export const InteractiveMap: React.FC<InteractiveMapProps> = ({ places, onSelectPlace }) => {
  ...
  const regionCounts = useMemo(() => {
    ...
    places.forEach(p => {
      ...
      else if (east) counts.east++;
      else counts.central++;
    });
    return counts;
  }, [places]);

  // Clicking a marker NEVER changes zoom or pan.
  ...
  // Wheel is the ONLY map zoom interaction.
  ...
  // Left mouse drag pans. A click without movement remains a click.
  ...
  const handleRegionClick = (regId: string) => {
    if (activeRegion === regId || (regId === 'east' && activeRegion === 'central')) {
      setActiveRegion('all');
    } else {
  ...
  const { clusters, singleMarkers } = useMemo(() => {
    ...
    for (let i = 0; i < mappedPlaces.length; i++) {
      ...
      for (let j = i + 1; j < mappedPlaces.length; j++) {
        ...
        if (Math.hypot(p1.svgX - p2.svgX, p1.svgY - p2.svgY) < distance) {
          group.push(p2);
          used.add(p2.id);
        }
      }
      if (group.length > 1) {
        ...
        clusterList.push({
          id: `cluster-${p1.id}`,
          x: group.reduce((a,p) => a + p.svgX, 0) / group.length,
          y: group.reduce((a,p) => a + p.svgY, 0) / group.length,
          places: group,
          ...
          <svg viewBox={`0 0 ${THAILAND_MAP_PROJECTION.width} ${THAILAND_MAP_PROJECTION.height}`} className="w-[360px] sm:w-[440px] md:w-[490px] lg:w-[530px] h-auto overflow-visible" style={{ maxHeight: '94%' }}>
            ...
            <g id="interactive-destination-markers-layer" pointerEvents="auto">
              {visibleClusters.map(cluster => {
                ...
                return (
                  <g key={cluster.id} transform={`translate(${cluster.x}, ${cluster.y})`} className="cursor-pointer" onClick={e => {
                    e.stopPropagation();
                    if (hasDraggedRef.current) return;
                    // Cluster click selects a place only. It NEVER zooms.
                    if (cluster.places.length) setSelectedPlace(cluster.places[0]);
                  }}>
                    <circle r="13" fill="transparent" />
                    <circle r="11" fill={style.color} stroke="#FFFFFF" strokeWidth="2" pointerEvents="none" />
                    <text textAnchor="middle" dy="3.5" fill="#FFFFFF" fontSize="9" fontWeight="800" fontFamily="sans-serif" pointerEvents="none">{cluster.places.length}</text>
                  </g>
                );
              })}
              ...
              {visibleSingleMarkers.map(place => {
                ...
                return (
                  <g key={place.id} transform={`translate(${place.svgX}, ${place.svgY})`} className="cursor-pointer" onClick={e => handleMarkerClick(place, e)} onMouseEnter={() => setHoveredPlace(place)} onMouseLeave={() => setHoveredPlace(null)}>
                    <circle r="14" fill="transparent" />
                    {isSelected && <circle r="12" fill="none" stroke={style.color} strokeWidth="2" strokeDasharray="3 2" pointerEvents="none" />}
                    <circle r={isSelected ? 7 : 5.5} fill={style.color} stroke="#FFFFFF" strokeWidth={isSelected ? 2.5 : 1.5} pointerEvents="none" />
                    <circle r={isSelected ? 2.5 : 1.8} fill="#FFFFFF" pointerEvents="none" />
                    {(isHovered || isSelected) && (
                      <g transform="translate(0, -16)" pointerEvents="none" className="select-none">
                        <rect x={-Math.min(getLocalized(place.name).length * 4.5 + 14, 75)} y="-17" width={Math.min(getLocalized(place.name).length * 9 + 28, 150)} height="20" rx="5" fill="#0F172A" opacity="0.95" />
                        <polygon points="-3,3 3,3 0,7" fill="#0F172A" opacity="0.95" />
                        <text x="0" y="-3.5" textAnchor="middle" fill="#FFFFFF" fontSize="9.5" fontWeight="700" fontFamily="sans-serif">{getLocalized(place.name)}</text>
                      </g>
                    )}
                  </g>
                );
        ...
        <div className="absolute inset-0 pointer-events-none p-4 sm:p-6 flex flex-col justify-between z-20">
          <div className="flex items-start justify-between gap-2">
            {(['north','northeast'] as const).map(reg => {
              const style = REGION_STYLES[reg];
              return <button key={reg} onClick={() => handleRegionClick(reg)} className={`pointer-events-auto flex items-center gap-2.5 px-3 py-2 rounded-2xl bg-white/95 border shadow-md text-left ${activeRegion === reg ? `border-${reg === 'north' ? 'emerald' : 'orange'}-500` : 'border-slate-200/90'}`}>
                <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: style.color }} />
                <div><p className="text-[11px] font-black text-slate-900">{lang === 'th' ? style.name.th : lang === 'zh' ? style.name.zh : style.name.en}</p><p className="text-[10px] font-semibold" style={{ color: style.color }}>{regionCounts[reg]} {lang === 'th' ? 'สถานที่' : lang === 'zh' ? '处景点' : 'places'}</p></div>
              </button>;
            })}
          </div>
          <div className="flex justify-end pr-2 sm:pr-6">
            <button onClick={() => handleRegionClick('east')} className="pointer-events-auto flex items-center gap-2.5 px-3 py-2 rounded-2xl bg-white/95 border border-slate-200/90 shadow-md text-left">
              <span className="w-3 h-3 rounded-full bg-purple-500 shrink-0" /><div><p className="text-[11px] font-black text-slate-900">{lang === 'th' ? 'ภาคตะวันออก' : lang === 'zh' ? '泰国东部' : 'Eastern Thailand'}</p><p className="text-[10px] font-semibold text-purple-700">{regionCounts.east} {lang === 'th' ? 'สถานที่' : lang === 'zh' ? '处景点' : 'places'}</p></div>
            </button>
          </div>
          <div className="flex items-end justify-between gap-2">
            <button onClick={() => handleRegionClick('south')} className="pointer-events-auto flex items-center gap-2.5 px-3 py-2 rounded-2xl bg-white/95 border border-slate-200/90 shadow-md text-left">
              <span className="w-3 h-3 rounded-full bg-sky-500 shrink-0" /><div><p className="text-[11px] font-black text-slate-900">{lang === 'th' ? 'ภาคใต้' : lang === 'zh' ? '泰国南部' : 'Southern Thailand'}</p><p className="text-[10px] font-semibold text-sky-700">{regionCounts.south} {lang === 'th' ? 'สถานที่' : lang === 'zh' ? '处景点' : 'places'}</p></div>
            </button>
            <button onClick={() => handleRegionClick('central')} className="pointer-events-auto flex items-center gap-2.5 px-3 py-2 rounded-2xl bg-white/95 border border-slate-200/90 shadow-md text-left">
              <span className="w-3 h-3 rounded-full bg-amber-500 shrink-0" /><div><p className="text-[11px] font-black text-slate-900">{lang === 'th' ? 'ภาคกลาง' : lang === 'zh' ? '泰国中部' : 'Central Thailand'}</p><p className="text-[10px] font-semibold text-amber-700">{regionCounts.central} {lang === 'th' ? 'สถานที่' : lang === 'zh' ? '处景点' : 'places'}</p></div>
            </button>
          </div>
        </div>
